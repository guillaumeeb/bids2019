...
Applications combining large data sets from different sources are most effective when deployed on cloud infrastructure.  This means that data must be accessible in a cloud or distributed storage friendly format, like Zarr for multi-dimensional data, Cloud optimized Geotiff for satellite imagery, or Parquet for tabular data. See \cite{b7} for more details on this crucial point.

\subsection{Ongoing work}
\label{ssec:ongowork}

Advancing both, the community and the technological stack, is currently done along the following lines:
\begin{itemize}
\item To facilitate work with the Pangeo software stack at scale, there is a Binder deployment\cite{b8} providing access to a cloud-based platform with a single click.
